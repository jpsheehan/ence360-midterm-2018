...
/**
 * read:	0 = there's nothing to read yet
 * 			1 = something to read
 * 
 * write:  	0 = channel is full, cannot write
 * 			1 = channel is empty, free for writing  
*/
...
void set_data(Channel *channel)
{
	...
	// wait for the channel to be empty (free for writing)
	...
	// tell the other threads there is something to read
...
void read_data(Channel *channel)
{
	...
	// we must wait for the reading semaphore to be 1 (something to read)
	...
	// now we say that the channel is empty (ready for writing)
